...
function CreateTimeMatrix(listOfTimes, offsetInWeeks = 0) {
  ...
  listOfTimes.forEach((time) => {
    if (
      counter < 5 &&
      time.date >= firstDayOfWeek &&
      time.date <= lastDayOfWeek
    ) {
      while (
        !CompareIfTimeIsEqual(
          GetOffsetDayOfWeek(firstDayOfWeek, counter),
          time.date,
        )
      ) {
        counter++;
...
function CreateTimeList(listOfTimes, offsetInMonths = 0) {
  ...
  listOfTimes.forEach((time) => {
    if (
      counter < monthEnd.getDate() &&
      time.date >= monthStart &&
      time.date <= monthEnd
    ) {
      while (
        !CompareIfTimeIsEqual(
          GetOffsetDayOfWeek(monthStart, counter),
          time.date,
        )
      ) {
        counter++;
      ...
      if (result[counter] === null) {
        result[counter] = new MonthTimeViewModel(
          time.from,
          time.to,
          time.state,
        );
...
function GetCurrentMonthData(timeTable, offsetInMonths = 0) {
  ...
  try {
    return [
      [
        ...(currMonth !== 0
          ? (timeTable[currYear][currMonth] ?? [])
          : (timeTable[currYear - 1][11] ?? [])
        ).filter((x) => x.date.getDate() >= 20),
        ...(timeTable[currYear][currMonth + 1] ?? []).filter(
          (x) => x.date.getDate() <= 19,
        ),
      ],
      fMonth,
...
function GetCurrentAndNextMonthData(timeTable, offsetInWeeks = 0) {
  ...
  try {
    return [
      ...timeTable[currYear][currMonth + 1],
      ...timeTable[nextInLineYear][nextInLineMonth + 1],
    ];
  } catch {
    return [];
  }
...
function AddSummaryRow(dataList, fMonth, sMonth) {
  dataList.sort((a, b) => a.date - b.date);
  const hourSum = dataList.reduce(
    (accumulator, currItem) => accumulator + currItem.duration,
    0,
  );
  return [
    ...dataList,
    new TimeModel(new DateModel(0, 0, 0), 0, 0, hourSum, "<-- Sum"),
    new TimeModel(new DateModel(0, 0, 0), 0, 0, 0, `From: ${fMonth}20.`),
    new TimeModel(new DateModel(0, 0, 0), 0, 0, 0, `To     : ${sMonth}19.`),
  ];
}
...
function GetFirstDayOfWeek(offsetInWeeks) {
  ...
  if (currentDay === 0 || currentDay === 6) additiveDaysForCorrectWeek = 7;

  const diff =
    currentDate.getDate() -
    currentDay +
    (currentDay === 0 ? -6 : 1) +
    additiveDaysForCorrectWeek;
